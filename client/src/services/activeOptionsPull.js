...
export async function activeOptionsPull(ticker) {
  ...
  const optionsData = await axios
    .get(`https://old.nasdaq.com/symbol/${ticker}/option-chain/most-active`)
    .then(response => {
      ...
      $('tbody tr').each(function(i, element) {
        ...
        result.optionType.push(
          $(element)
            .find('td.most-active-col1')
            .next('td')
            .text()
        )
        result.strikePrice.push(
          $(element)
            .find('td.most-active-col1')
            .next('td')
            .next('td')
      ...
      result.optionType = result.optionType.filter(el => {
        return el != ''
      })
      result.strikePrice = result.strikePrice.filter(el => {
        return el != ''
      })
      result.contractPrice = result.contractPrice.filter(el => {
        return el != ''
      })
      result.todayChange = result.todayChange.filter(el => {
        return el != ''
      })
      result.volume = result.volume.filter(el => {
        return el != ''
      })
      result.openInt = result.openInt.filter(el => {
        return el != ''
      })
      ...
      for (let x = 0; x < result.optionType.length; x++) {
        ...
        newResultRow.push(
          result.dates[x],
          result.optionType[x],
          result.strikePrice[x],
          result.contractPrice[x],
          result.todayChange[x],
          result.volume[x],
          result.openInt[x],
          result.volOI[x]
        )
